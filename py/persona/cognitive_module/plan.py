...
class Plan:
    ...
    def plan(persona, personas, new_day, retrieved):
        ...
        #               3) False
        if focused_events:
            reaction_mode = _should_react(persona, focused_events, personas)
            if reaction_mode:
                if reaction_mode[:9] == "chat with":
                    _chat_react(persona, focused_events, reaction_mode, personas)
                    pass
...
def _plan_daily_schedule(persona, new_day):
    ...
    persona.a_mem.add_thought(created, expiration, sub, pre, obj,
...
def generate_task_decomp(persona, task, duration):
  ...
  return LLMManager.run_prompt_task_decomp(persona, task, duration)[0]
...
def generate_action_sector(act_desp, persona):
  ...
  return LLMManager.run_prompt_action_sector(act_desp, persona)[0]
...
def generate_action_arena(act_desp, persona, act_world, act_sector):
  ...
  return LLMManager.run_prompt_action_arena(act_desp, persona, act_world, act_sector)[0]
...
def generate_act_obj_desc(act_game_object, act_desp, persona):
    ...
    return LLMManager.run_prompt_act_obj_desc(act_game_object, act_desp, persona)[0]
...
def generate_convo_summary(persona, convo):
    convo_summary = LLMManager.run_prompt_summarize_conversation(persona, convo)[0]
    return convo_summary


def generate_decide_to_talk(init_persona, target_persona, retrieved):
    x = LLMManager.run_prompt_decide_to_talk(init_persona, target_persona, retrieved)[0]
...
def generate_decide_to_react(init_persona, target_persona, retrieved):
    ...
    return LLMManager.run_prompt_decide_to_react(init_persona, target_persona, retrieved)[0]
...
def generate_new_decomp_schedule(persona, inserted_act, inserted_act_dur, start_hour, end_hour):
    ...
    return LLMManager.run_prompt_new_decomp_schedule(persona,
                                              main_act_dur,
                                              truncated_act_dur,
                                              start_time_hour,
                                              end_time_hour,
